...
```mermaid
flowchart TB
    D["发现<br/>读取 Skill、Tool、Agent 元数据"] --> N["规范化<br/>写入能力注册表"]
    N --> R["受控召回 / 初筛<br/>规则、关键词、语义"]
    R --> E["权威暴露策略<br/>身份、阶段、风险、可用性"]
    E --> P["最小候选集<br/>交给规则或模型选择"]
    P --> K{"选中的能力类型"}
    K --> C["Skill / Resource / Prompt<br/>权限、信任、大小门 -> Context Builder"]
    K --> V["Tool<br/>参数、对象授权、预算、审批门"]
    K --> A["A2A<br/>身份、委派合同、数据与取消门"]
```
