...
> 本章解释模型怎样提出一个结构化动作、谁真正执行它，以及结果怎样回到下一轮上下文。读完后，你应能把“模型说它查过了”与“系统确实执行过一次可追踪、可授权的查询”区分开。若还不熟悉模型、Harness、Skill 与 MCP 的基本关系，先阅读[基础关系](03-foundations.md)。

> **两遍阅读建议：** 第一次读到“一次调用的完整闭环”，再看“相邻概念的边界”和本章小结，先掌握主链；第二次再读三家 API、并行、五层校验和概念实现，把它们作为生产化选读。
...
**Function Calling 这个名字容易造成误解。模型通常没有直接进入你的进程调用函数；它返回的是一个结构化的调用提议。** Harness 或应用仍要校验、授权、调度并回填结果，具体执行方可能是本地代码、厂商服务或远端能力。Anthropic 常用 `Tool Use` 表达同类机制；本章在不讨论厂商专有差异时，把二者统称为“工具调用”。
...
这个对象不是查询结果，也不能证明查询已经发生。`correlation_id` 是本教程概念信封中的中立关联标识：协议提供调用 ID 时沿用并校验；协议不提供时，由 Harness 创建内部 ID，只保存在任务状态和 Trace 中。内部 ID 不能伪装成厂商 API 字段。Tool 名称回答“想调用什么”，参数回答“想怎样调用”，关联标识回答“内部怎样追踪这次提议”。
