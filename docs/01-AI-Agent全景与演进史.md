...
这张表不替代经典 Agent 或对话系统课程，只用于划清范围：2017 年之后的 LLM Agent 把通用自然语言模型接入了既有的 Agent 和自然语言接口问题，而不是从零发明“行动者”或“自然语言到参数”。代表性来源见[来源索引 A00](24-官方来源事实标签与版本基线.md#a00-经典智能体研究范围)与[A00A](24-官方来源事实标签与版本基线.md#a00a-任务型对话与语义接口前史)。
...
这里采用“**Agent = 模型 + 运行时提供的上下文、状态、工具和执行循环**”这条简化关系建立直觉。这不是某项开放规范规定的公式，而是用于强调三个边界：模型生成一个工具调用建议，不等于工具已经执行；模型读到一段检索结果，不等于它拥有知识库；系统可以循环，也不等于它应获得无限自主权。更完整的基础定义见[03. 先认识 Agent、Skill 与 MCP](03-Agent-Skill-MCP基础关系.md)。
...
| 2026-07-10 | 本系列状态截面 | 四条路线汇合 | 截至核对日，MCP、A2A 与 Agent Skills 都已有公开规范或仓库，但版本、实现范围和治理方式不同。当前基线集中记录在[24. 官方来源、事实标签与版本基线](24-官方来源事实标签与版本基线.md)。 |
...
模型能力路线本身也不是一个“参数越大就解决全部问题”的故事。上下文学习让任务说明和少量示例在请求内发挥作用；指令微调与人类偏好训练改善模型遵循要求的倾向；思维链和后续推理时计算探索怎样为难题投入更多步骤。它们都不能授予环境权限、修复缺失证据或证明外部动作发生。Token、自回归生成、训练与后训练、长上下文、多模态和推理模型的完整基础见[02. LLM 能力底座](02-LLM能力底座与模型选型.md)。

RAG 把一个新边界放到模型参数之外：**运行时在生成前或生成过程中，从外部知识源选择材料，再把它们提供给模型。** 这让更新知识不必都进入模型参数，也让来源追踪成为可能。它没有自动解决切块、召回、重排、权限、时效和引用问题，更不等于长期 Memory。知识和状态的完整工程链见[06. Context Engineering、RAG 与 Memory](06-上下文工程RAG与Memory.md)。
...
因此不能写成“Toolformer 发明了 Function Calling”，也不能把 ReAct 等同于所有 Agent Loop。它们提供了研究方法和设计语言；真正执行动作、保存状态和控制循环的仍是应用运行时。ReAct、Planning 与 Workflow 的工程取舍见[05. Agent Loop、Workflow 与 Planning](05-Agent循环工作流与规划.md)。
...
模型通常不直接运行函数。完整数据流和 Schema 设计见[04. Function Calling 与 Tool Use](04-Function-Calling与Tool-Use.md)。
...
MCP 不负责模型一定选对 Tool，不自动授予业务权限，也不把 Server 返回的文字升级成可信指令。Server 提供能力，Host/Harness 决定暴露什么、何时请求审批以及怎样把结果加入上下文。协议结构和最小 Server 实践见[11. 从零制作一个高质量 MCP Server](11-高质量MCP-Server制作.md)。
...
一个 Agent 可以经 A2A 接收任务，再在内部通过 MCP 调用工具。A2A 不替代 MCP；同一进程内的两个子 Agent 若已有可靠调度接口，也不必强行使用 A2A。多 Agent 的委派、权限和交接风险见[07. Multi-Agent、委派与 A2A](07-Multi-Agent委派与A2A.md)。
...
安装 Skill 不会修改模型参数，也不会自动获得文件、网络或业务系统权限。Skill 可以要求在某一步查询制度，但实际能力仍由本地 Tool 或 MCP 提供，执行仍由 Harness 管理。制作方法见[10. 从零制作一个高质量 Agent Skill](10-高质量Agent-Skill制作.md)，二者怎样组合见[14. Skill 与 MCP 组合实践](14-Skill与MCP组合实践.md)。
...
1. **第一次接触 Agent**：[01. 全景与演进](01-AI-Agent全景与演进史.md) -> [02. LLM 能力底座](02-LLM能力底座与模型选型.md) -> [03. 基础关系](03-Agent-Skill-MCP基础关系.md) -> [04. Function Calling](04-Function-Calling与Tool-Use.md) -> [05. Agent Loop](05-Agent循环工作流与规划.md)。
2. **要解决知识与状态问题**：[02. LLM 能力底座](02-LLM能力底座与模型选型.md)中的上下文与 Embedding -> [06. Context Engineering、RAG 与 Memory](06-上下文工程RAG与Memory.md) -> [08. 能力发现与路由](08-能力发现候选裁剪与路由.md) -> [11. MCP Server](11-高质量MCP-Server制作.md)中 Resources 与 Tools 的区别。
3. **要做可复用专项能力**：[10. Agent Skill](10-高质量Agent-Skill制作.md) -> [14. Skill 与 MCP 组合实践](14-Skill与MCP组合实践.md)。
4. **要做跨系统协作**：[07. Multi-Agent、委派与 A2A](07-Multi-Agent委派与A2A.md)；先证明本地单 Agent 或 Workflow 不足，再引入远程委派。
5. **要交付团队或生产环境**：[09. 人机协作与可控交互](09-人机协作与可控交互.md) -> [15. 生产级 Agent Runtime](15-生产级Agent-Runtime架构.md) -> [12. 跨 Harness 适配](12-跨Harness适配.md) -> [13. 质量工程与安全](13-质量工程与安全治理.md) -> [24. 官方来源、事实标签与版本基线](24-官方来源事实标签与版本基线.md)。
