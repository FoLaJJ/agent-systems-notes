...
> 本章是选读。它不是“追热点清单”，而是帮助团队判断 Agent 前沿方向是否值得学习、试点，或者进入内部规范。结论默认带有时效性：能进主线的东西，最后必须回到前面章节的责任边界、数据流、评测和安全门。
...
这里有个很实用的判断：Runtime 越像“操作系统”，越不能只看模型能力演示。演示里一个漂亮的浏览器动作，落地时会变成权限、日志、撤回、隔离和责任人的组合题。
...
这对写 Skill 很重要：Skill 不应鼓励 Agent 为每件事“自主探索”。好的 Skill 应该把确定部分写成流程，把语义判断写成证据标准，把危险动作交给 Harness 和业务系统。自主性听起来很性感，生产里通常先死在预算、权限和撤回上。
...
对团队来说，关键问题不是“RAG 是否过时”，而是知识从创建、审核、索引、检索、注入、引用、过期到删除的链路是否清楚。RAG 这类东西最容易被写成玄学，其实问题通常出在证据链：来源不稳、召回不全、注入不克制、引用不可查。
...
这也是为什么本教程把路由评测、轨迹证据、安全用例和用途治理分开讲。前沿的自动评分、Trace Grading 或模型评审都只能降低评测成本，不能替代黄金用例、人工复核和业务指标。评测如果只看最终答案，很容易把“运气好”误判成“系统稳”。
...
| 多 Agent 群聊决策 | 看起来接近团队讨论 | 容易互相强化错误，责任和终止条件不清 |
| 去中心化 Agent 网络 | 可跨组织协作 | 治理、信任、隐私和法律责任尚不成熟 |

本教程的建议是：这些方向可以作为研究和小范围原型，但不进入内部稳定主线，除非能回答身份、授权、证据、预算、撤回和责任六个问题。自治 Agent 最容易让人兴奋，也最容易让系统负责人晚上睡不着。
...
- 值得采用的前沿能力，最后都必须回到稳定问题：谁执行、谁授权、谁记录、谁验证、谁负责。
