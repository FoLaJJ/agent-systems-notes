...
> Agent 不是凭空产生动作的黑盒。它依赖一个经过预训练和后训练、在给定上下文中逐步生成结果的模型。本章不讲神经网络推导，而是建立工程上够用的模型心智：模型为什么能遵循指令、怎样生成 Tool Call、为什么会稳定地犯错，以及 Harness 应怎样利用而不是迷信模型能力。

> **阅读目标：** 读完后，你应能区分模型训练、模型请求和 Agent Runtime，解释 Token、采样、上下文窗口、指令微调、偏好对齐、推理时计算、多模态与 Embedding 的作用，并能把 Agent 故障定位到正确层次。
...
现代 LLM 能力来自多条路线叠加。下表是学习地图，不是完整模型史；日期与最窄可证明结论见[来源索引](24-sources.md#ai-agent模型检索推理与行动研究)。
...
`[研究]` InstructGPT 展示了监督微调与人类反馈训练改善指令遵循的一条公开路线；DPO 提供了另一类直接偏好优化方法；知识蒸馏则早于现代 LLM。它们是不同训练方法，不应统称为某一个产品的“对齐开关”。来源见 [A12](24-sources.md#a12-指令微调与人类反馈)、[A16](24-sources.md#a16-直接偏好优化)与[A17](24-sources.md#a17-知识蒸馏)。
...
一个高质量 Prompt 不是一段“神奇咒语”，而是模型请求中一组有来源、有优先级、有版本的合同。可以把请求拆成六块：
...
`[研究]` “Lost in the Middle”展示了模型使用长输入时与相关信息位置有关的性能变化。它不证明所有模型或任务具有相同曲线，但足以反驳“只要没超过窗口就等于都读懂了”。工程上仍要选择、排序、压缩和追踪来源，详见[Context Engineering](06-context-rag-memory.md)。
...
`[研究]` 推理时计算缩放工作比较了不同推理计算分配策略，并说明收益依赖问题难度、验证器和计算分配方式。它支撑“测试时可以增加计算”在现代 LLM 中受到集中研究这一节点，不表示搜索、采样或测试时计算在 2024 年才被发明，也不证明任意模型只要延长输出就会单调变好，来源见 [A19](24-sources.md#a19-推理时计算缩放)。
...
多模态模型可以在一次任务中处理文本、图像、音频或视频等两种或多种输入。每个产品只支持其中一个子集，输入与输出能力也可能不同，必须分别核对支持矩阵；本章只讨论 Agent 的多模态观察，不展开生成不同模态产物。对 Agent 而言，多模态扩展了观察表面：读取截图、识别界面、理解图表、转录会议或检查扫描件。代表性输入理解技术报告见 [A20](24-sources.md#a20-多模态模型能力边界)，但它不能单独证明所有多模态产品具有相同能力。
...
Embedding（向量表示）把文本、图像或其他对象映射为一组数值，使相近内容在某种训练语义下更容易被检索或聚类。它通常不负责生成最终回答，而是服务于 RAG、能力召回、去重和相似案例查找。文本句向量、非对称文本检索与图文联合表示分别可参考 [A15](24-sources.md#a15-embedding-与语义检索)、[A21](24-sources.md#a21-非对称双编码检索)和[A22](24-sources.md#a22-跨模态向量表示)。

相似度只说明“在该表示空间中接近”，不证明内容真实、最新、权威或当前主体有权读取。Embedding 适合召回候选，权限、关键词、时间和业务规则仍要在模型之外处理。查询/文档编码器对、索引迁移和 Recall@k 评测等实现细节集中见[Context Engineering、RAG 与 Memory](06-context-rag-memory.md#embedding把对象映射到可比较的表示)，避免把模型直觉与 RAG 运维重复两遍。

## 不要把所有失败都叫“幻觉”

这是本教程最重要的诊断原则之一。只有先定位失败层，修复才不会变成“换一个更大模型”。
...
“幻觉率”作为单一总指标会把这些问题混在一起。评测应分别观察事实、检索、路由、参数、执行、状态和业务结果，详见[质量工程与安全](13-quality-and-security.md)。
...
- [AI Agent 全景与演进史](01-agent-evolution.md)：把模型路线放回知识、工具、过程和协议的并行历史；
- [模型请求、Harness 与上下文](03-foundations.md)：理解这些能力怎样被一次请求真正使用；
- [Function Calling 与 Tool Use](04-function-calling.md)：追踪 Tool Call 的消息与执行闭环；
- [Context Engineering、RAG 与 Memory](06-context-rag-memory.md)：设计证据选择、Embedding 检索和状态生命周期；
- [人机协作与可控交互](09-human-agent-interaction.md)：把不确定模型放进可理解、可纠正的用户体验；
- [质量工程与安全](13-quality-and-security.md)：把模型能力变成可重复的组件、轨迹和业务评测；
- [官方来源与版本基线](24-sources.md)：核对本章论文与规范来源。
