...
> 核心问题是：模型在当前这一步究竟应该看到什么。内容区分上下文、RAG、Memory、Skill 与 MCP，并建立一条从信息选择、检索、写入到过期删除的完整数据链。

![上下文工程流水线中文图](../assets/images/context-pipeline-zh.svg)
...
`[建议]` 上下文质量可以理解为“在预算内最大化有效证据”，而不是最大化字符数量。上下文窗口更大，只是容量增加，并不会自动解决冲突、过期、权限或注意力分散。
...
`[建议]` 不把“RAG”限定为“向量库 + top-k”。真正可维护的知识型 Agent 应拆成三层：
