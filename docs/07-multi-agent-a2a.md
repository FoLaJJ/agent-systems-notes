...
一次复杂发布同时包含数据库迁移、权限变更、前端灰度和合规要求。让一个 Agent 从头处理可行，但它可能需要加载过多工具和知识，也很难让数据库、安全与业务判断保持独立。

另一种设计是：协调 Agent 固定审查边界，再把独立问题分别委派给专项 Agent，随后合并证据。
...
sequenceDiagram
    ...
    loop 长任务期间
      R-->>C: WORKING / INPUT_REQUIRED / AUTH_REQUIRED + 可选 Artifact
      C->>R: 补充 Message、完成授权、查询状态或请求取消
    end
    R-->>C: COMPLETED / FAILED / CANCELED / REJECTED + 可选 Artifact
```
