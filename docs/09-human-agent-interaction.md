...
> 一个 Agent 即使内部架构正确，如果用户不知道它正在做什么、依据什么、能否中断，以及哪一步真的执行过，仍然不是高质量系统。本章把澄清、计划、审批、进度、证据、不确定性、纠正、取消、恢复和人工交接组织成一套人机合同。

> 本章讨论交互原则，不规定某个产品必须使用哪种按钮或界面。聊天、IDE、命令行、工单机器人和后台任务可以有不同界面，但应提供同样关键的控制与证据。
...
| 问题 | 用户需要看到什么 | 系统真实依据 |
| --- | --- | --- |
| 你理解的目标是什么 | 对象、环境、范围、成功条件 | 结构化任务目标与当前状态 |
| 你准备怎么做 | 简洁步骤、将使用的数据和能力 | 版本化计划摘要，不是内部思维全文 |
| 你现在能做什么 | 只读、写入、外发、委派及限制 | 当前身份、Scope、策略和能力目录 |
| 哪一步需要我决定 | 具体对象、影响、选项和有效期 | 风险策略与 Approval 记录 |
| 当前进行到哪里 | 阶段、已完成项、等待原因和预计下一事件 | Run / Step / Call 状态 |
| 你的结论依据什么 | 来源、观察时间、缺口和冲突 | Evidence / Artifact 引用 |
...
一个好的澄清问题应一次解决一个决策，并给出差异清楚的选项。例如：“你指的是生产环境 `orders-ap-southeast-1`，还是测试环境 `orders-staging`？”比“请提供更多信息”更可行动。
...
| 仅提议动作 | 明确尚未执行及所需决定 | “建议创建草稿，等待你的批准” |
...
接手人需要知道目标、当前状态、已验证证据、已发生动作、未决决定和责任边界。默认不传内部思维全文、凭据、其他租户数据或与决定无关的历史。若交给另一个 Agent，仍要使用委派合同、最小权限和 Artifact 验收，详见[Multi-Agent 与 A2A](07-multi-agent-a2a.md)。
...
- [LLM 能力底座](02-model-capabilities.md)：理解模型能力和错误从哪里来；
- [Agent Loop、Workflow 与 Planning](05-agent-loop-workflows.md)：把交互节点落到状态、预算和停止条件；
- [Multi-Agent、委派与 A2A](07-multi-agent-a2a.md)：设计跨 Agent 交接合同；
- [质量工程与安全](13-quality-and-security.md)：测试批准、注入、身份、数据与用途治理；
- [生产级 Agent Runtime](15-production-agent-runtime.md)：实现持久状态、取消对账和在途迁移；
- [官方来源与版本基线](24-sources.md)：核对人本设计与风险治理来源。
