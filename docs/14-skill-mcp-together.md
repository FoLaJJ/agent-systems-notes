...
sequenceDiagram
    ...
    participant H as Harness
    participant L as 模型
    participant M as Change MCP
    U->>H: 根据报告创建草稿变更单
    H->>L: 风险报告、证据引用、预览能力
    L-->>H: 草稿字段提议
    H->>M: change.preview_draft(字段)
    M-->>H: plan_id + digest + 预览
    H-->>U: 展示对象、字段、影响与有效期
    U-->>H: 批准计划摘要
```
...
sequenceDiagram
    ...
    actor U as 用户
    H->>H: 记录 pending Call + idempotency_key
    H->>M: change.create_draft(plan_id, digest, key)
    M->>B: 使用真实主体创建草稿
    B-->>M: change_id + 状态
    M-->>H: created / already_created + change_id
    H->>M: get_change_status(change_id)
