...
```mermaid
flowchart TB
    U["用户目标"] --> P["过程与协作<br/>Workflow、Planning、Skill、委派、A2A"]
    P --> H["Agent Runtime / Harness<br/>上下文、能力路由、状态、权限、预算、执行循环"]
    H <--> L["模型推理<br/>理解、生成、规划、选择候选动作"]
    H <--> K["知识与状态<br/>Context、RAG、Memory"]
    H <--> T["工具与连接<br/>Function Calling、Plugin、MCP"]
    T <--> E["文件、API、数据库与业务系统"]
    P <--> A["其他 Agent 系统"]
    H <--> Q["横切治理<br/>Eval、安全、可观测性、成本"]
    H --> O["回答、动作结果或任务产物"]
```
...
flowchart LR
    subgraph R1["模型推理路线"]
      L1["文本生成"] --> L2["指令与推理提示"] --> L3["ReAct<br/>推理与行动交错"] --> L4["规划、反思与受控决策"]
    end
    subgraph R2["知识与状态路线"]
      K1["Prompt / 当前材料"] --> K3["Context Engineering<br/>选择、排序与裁剪"]
      K2["RAG<br/>按需检索外部证据"] --> K3
      K4["Memory<br/>跨步骤或会话状态"] --> K3
    end
    subgraph R3["工具与连接路线：不同抽象分支"]
      T1["应用专用 API"] --> T2["模型使用外部能力"]
      T2 --> T3["Toolformer<br/>训练方法"]
      T2 --> T4["Plugins<br/>产品扩展"]
      T2 --> T5["Function Calling<br/>模型-应用接口"]
      T2 --> T6["Structured Outputs<br/>结构合同"]
      T2 --> T7["MCP<br/>能力连接协议"]
      T5 -. "参数结构可组合" .-> T6
      T5 -. "由 Harness 接力" .-> T7
    end
    subgraph R4["过程与协作路线：并行分工"]
      P1["固定 Prompt 与脚本"] --> P2["Workflow / Agent Runtime"]
      P2 --> P3["Agent Skills<br/>过程知识"]
      P2 --> P4["Multi-Agent / A2A<br/>委派与协作"]
    end
    L3 -. "通过动作与观察接口结合" .-> T2
    K2 -. "由运行时检索" .-> P2
    T7 -. "由运行时授权和调度" .-> P2
...
```mermaid
flowchart LR
    A["01 全景与演进<br/>先知道问题地图"] --> L["02 模型能力<br/>生成、训练与推理"]
    L --> B["03 基础<br/>模型请求、Harness、Skill、MCP"]
    B --> C["04 工具调用<br/>Function Calling"]
    B --> D["05 执行控制<br/>Loop、Workflow、Planning"]
    B --> E["06 知识状态<br/>Context、RAG、Memory"]
    B --> F["07 系统协作<br/>Multi-Agent、A2A"]
    C --> R["08 能力发现与路由"]
    E --> R
    F --> R
    R --> X["09 人机协作<br/>澄清、批准、纠正与恢复"]
    R --> G["10 制作 Skill"]
    D --> G
    R --> H["11 制作 MCP Server"]
    G --> I["14 Skill + MCP<br/>完整任务实践"]
    H --> I
    I --> Y["15 生产 Runtime<br/>持久任务与运行治理"]
    Y --> J["12 跨 Harness 适配"]
    J --> K["13 质量、安全与发布门"]
    K --> Z["24 来源、版本与维护"]
```
