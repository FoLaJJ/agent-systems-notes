...
> 目标是把一个 Skill 写到能被稳定发现、正确触发、按流程执行，并产出可验收结果。先理解 Skill 为什么出现，再写出一个只有 `SKILL.md` 的最小 Skill，随后补上路由、执行流程、参考资料、失败处理和团队级质量要求。
...
`[平台]` 2025 年 10 月，Anthropic 公开介绍由 `SKILL.md`、脚本和资源组成并按需加载的 Agent Skills 机制；同年 12 月宣布其成为开放标准。这个节点证明了当前文件格式和渐进披露机制的公开发布，不表示“把流程写成可复用说明”到那时才出现。可核对时间线见[AI Agent 全景与演进史](01-AI-Agent全景与演进史.md#2017-2026可证实的关键节点)。
...
这些是产品的当前发现路径，不是开放格式的一部分。个人级目录、显式调用方式和优先级见[跨 Harness 适配](12-跨Harness适配.md)。第一次尝试时，选择当前使用的平台，把整个 `release-note-writer` 目录放进去，然后开启一个新会话。
...
先在 Harness 的 Skill 列表或激活提示中确认它已被发现。预期结果是：第一条明确点名 Skill，第二条能根据请求含义选择这个 Skill，第三条不应因为出现“发布”二字就误用它。这里的“路由”就是 Harness 和模型根据请求选择相关 Skill 的过程；发现、资格过滤、候选召回和最终选择的完整分层见[能力发现、候选裁剪与路由](08-能力发现候选裁剪与路由.md)，显式调用语法因平台而异，见[跨 Harness 适配](12-跨Harness适配.md)。
...
Skill 的正文也是给模型看的指令，但 Skill 多了发现、路由、资源组织和版本维护这一层。一个成熟 Skill 可以理解为四份合同：
...
如果一段规则几乎对仓库中的每次任务都适用，它更适合放进 `AGENTS.md`、`CLAUDE.md` 等项目指令；如果它需要实时访问外部系统，应使用 MCP 或 Harness 已有工具。完整选型见[第一章](03-Agent-Skill-MCP基础关系.md#prompt项目指令skillmcp-和-plugin-怎么选)。

Skill 也不等于知识库或长期记忆：大量动态资料适合通过 RAG 检索，跨步骤或跨会话状态适合由 Memory 管理，Skill 则保存一类任务相对稳定的方法和资源入口。三者的选择见[Context Engineering、RAG 与 Memory](06-上下文工程RAG与Memory.md#promptragmemoryskillmcp-与-fine-tuning-怎么选)。
...
| 问题 | 好答案 | 坏答案 |
| --- | --- | --- |
| 使用者是谁 | 第一次被路由到该 Skill 的 Agent，需要按步骤完成任务 | “资深专家”这类空泛角色 |
...
正文可以写成“任务状态机”，而不是一长段建议。状态机能让人看到正常路径、缺证路径和风险路径分别怎样结束：
...
能用的 Skill 更像“给 Agent 的可执行流程说明”，不是面向人类的长篇经验总结。最可靠的写法，是先看没有 Skill 时 Agent 会怎样失败，再把失败收敛成规则、分支和检查点：
...
这个方法的关键不是制造复杂评测，而是让每条规则都有来处。凡是没有对应失败模式、没有复用价值、也不能改善验收的句子，都应该谨慎加入；它们会占用上下文，并可能和真正重要的步骤互相干扰。成熟 Skill 拼的是失败样本是否被充分理解，而不是正文辞藻。
...
不要只写“参考 references 目录”，也不要要求每次预先通读所有资料。

### 确有确定性工作时再添加脚本资源

反复计算、格式转换、静态校验等任务在真实 Skill 中可以交给脚本资源。模型负责判断“何时运行”，脚本负责稳定地产生结果。脚本应有明确参数、退出状态和错误信息，默认不执行危险写操作。这里解释这种分层，不在仓库里放脚本。
...
前面的发布说明 Skill 已经展示了基本骨架。现在继续对照仓库中的[发布风险审查 Skill](16-案例发布风险审查Skill.md)，观察一个最小 Skill 怎样成长为包含分支、证据与安全边界的团队流程：

1. `description` 区分风险审查、执行部署、编写发布说明和事故响应；
2. 正文先固定版本、环境、基线和证据截止时间；
3. 证据被标为已验证、间接、过期、缺失或冲突；
4. 评分前按需读取[风险模型](17-案例发布风险评分模型.md)；
5. 需要当前制度时，只描述“查询发布制度”这项语义能力，也就是用业务含义表达所需能力，而不写死某个 Harness 的工具前缀；这是跨 Harness 设计建议，不是 Agent Skills 规范字段；
...
本仓库把[路由案例与相邻候选夹具](19-案例Skill路由评测.md)整理成文档附录。第一次学习不必运行自动评测，先用新会话人工尝试正反例；准备交付团队时，再按照[质量工程与安全](13-质量工程与安全治理.md)建立可重复评测。
...
可移植核心只依赖开放字段、相对路径和语义能力。安装目录、展示元数据、预授权工具和平台命令放进适配层。完整矩阵见[跨 Harness 适配](12-跨Harness适配.md)。
...
- [Agent 如何发现、加载与注入上下文](03-Agent-Skill-MCP基础关系.md#skill-怎样进入上下文)
- [Context、RAG、Memory 与 Skill 的边界](06-上下文工程RAG与Memory.md)
- [Skill、Tool 与 Agent 的统一能力路由](08-能力发现候选裁剪与路由.md)
- [四类 Harness 的目录、调用与适配方式](12-跨Harness适配.md)
- [路由评测、权限和供应链安全](13-质量工程与安全治理.md)
- [Skill 怎样编排 MCP](14-Skill与MCP组合实践.md)
- [Agent Skills 开放规范与官方资料](24-官方来源事实标签与版本基线.md#agent-skills历史与开放规范)
