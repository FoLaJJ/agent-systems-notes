...
```mermaid
flowchart TB
    A["模型推理<br/>理解目标并提出下一步"]
    B["知识与状态<br/>选择证据并保存受控状态"]
    C["工具与连接<br/>表达动作并接入外部能力"]
    D["过程与协作<br/>控制多步任务与委派"]
...
四条路线内部可以拆开看，避免把不同抽象写成一条替代链：

```mermaid
flowchart LR
    A1["预训练与生成"] --> A2["指令遵循"] --> A3["ReAct / 推理时计算"]
    B1["Prompt 材料"] --> B2["RAG"] --> B3["Context / Memory"]
    C1["API / Plugin"] --> C2["Function Calling"] --> C3["MCP"]
    D1["Workflow"] --> D2["Agent Runtime"] --> D3["Skills / A2A"]
```
