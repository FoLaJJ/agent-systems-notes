...
这份文档不从框架 API 和配置字段切入。先把职责地图和演进史讲清楚：模型为什么能提出 Tool Call，为什么也会流畅地犯错，一次模型请求、能力选择与任务执行中的数据到底怎样流动。随后再进入 Skill、MCP Server、多 Agent、人机协作、生产 Runtime、跨平台、质量、安全与用途治理。
...
通用模型可以写出检查项，但它不知道你们公司的审查流程，读不到最新发布制度，也不能自己证明某次查询或部署已经执行。要把“看起来会分析”变成“能被团队信任的判断”，中间需要一套受控系统：
...
| **Function Calling** | 用工具名、参数和平台规定的调用绑定表达“希望查询哪项制度” | 规范填写的办事申请单 |
...
这些概念不是竞争关系。模型提出 Tool Call 不等于动作已执行，Skill 没有天然外部权限，MCP 不会替 Agent 决定工作方法，A2A 也不适合包装一个简单函数；真正组织它们的是 Harness。这里如果混成一锅粥，后面所有“Agent 架构设计”都会变成玄学。
...
flowchart LR
    subgraph R1["模型推理"]
      A1["预训练与文本生成"] --> A2["指令遵循 / 上下文学习"] --> A3["ReAct / 推理时计算"]
    end
    subgraph R2["知识与状态"]
      B1["Prompt 内材料"] --> B2["RAG"] --> B3["Context Engineering / Memory"]
    end
    subgraph R3["工具与连接"]
      C1["专用 API / Plugin"] --> C2["Function Calling"]
      C2 --> C3["Structured Outputs<br/>收紧结构合同"]
      C2 --> C4["MCP<br/>标准化能力连接"]
    end
    subgraph R4["过程与协作"]
      D1["固定脚本 / Workflow"] --> D2["Agent Runtime"]
      D2 --> D3["Agent Skills<br/>封装过程知识"]
      D2 --> D4["Multi-Agent / A2A<br/>处理委派协作"]
    end
```
...
很多资料会告诉你文件放在哪里、命令怎样运行。这份文档更关心那些真正决定质量的问题：
...
我们不把“模型选工具”视为一个黑箱。Skill、Tool、MCP 与 A2A 统一做发现、候选治理、Trace 和评测，但选中后分别进入上下文加载、Tool 执行或任务委派。受控目录可以先召回 ID，也可以先做授权感知过滤；不变量是敏感元数据暴露前经过模型外的权威策略，具体调用或委派再做一次授权。

### 6. 跨平台追求行为一致，而不是文件完全相同

Claude Code、Codex CLI、Gemini CLI、GitHub Copilot CLI 和 VS Code 的发现目录、显式调用、权限界面并不完全一致。我们的做法是：
...
我们区分无依据生成、检索漏召回、路由误选、Tool 参数错误、虚构执行、状态漂移和 Runtime 竞态。它们表面都可能是一句错误回答，却不能都靠“换更强模型”修复。

### 9. 把 Agent 架构当作状态控制问题

ReAct、Plan-and-Execute、Supervisor、Group Chat、Swarm、Pipeline、Graph Runtime 不是同一层概念。教程会先说明谁控制下一步、状态放在哪里、谁合并结果、如何停止和恢复，再讨论 LangChain、LangGraph、AutoGen 等框架里的对应范式。

### 10. 把 RAG 争议拆成知识治理与上下文注入

“RAG 被淘汰”通常混淆了三件事：长上下文降低部分检索压力，Wiki/知识库改善知识治理，Agentic/GraphRAG 扩展检索形态。我们的结论是：Wiki 是上游知识治理层，RAG 是运行时证据选择与上下文注入层，二者应组合，而不是互相替代。
...
### 路线 B：我想学 Function Calling 和 Agent Runtime
...
6. 最后用[质量工程与安全](docs/13-quality-and-security.md)设计轨迹评测、用途治理和可观测性。
...
### 路线 G：我要建设公司内部 Agent 能力平台

按 `01 -> 02 -> 03 -> 04 -> 05 -> 06 -> 08 -> 09 -> 10 -> 11 -> 12 -> 13 -> 14 -> 15 -> 20 -> 21 -> 24 -> 25` 阅读，并重点使用两个评审模板：
...
章节编号已经改成连续主线：基础原理在前，制作教程居中，案例章节、评审模板、维护和来源放在后半部分。推荐阅读顺序以“学习路线”为准。
...
| [23. 贡献与维护约定](docs/23-maintenance-guide.md) | 章节归属、事实分级、示例维护和发布检查约定 | 参考 |
...
先用两个问题做快速判断：**你缺的是做事方法，还是外部能力？内容需要稳定复用，还是必须读取当前状态？**
...
为了不把规范、产品行为、研究结果和我们的观点混在一起，内容使用五种标签：
...
案例章节、路由案例、评审表和维护约定都已经并入 `docs/`，避免 GitHub 入口被多个目录分散。
...
6. 只有准备交付团队时，才要求读者进入完整评测、安全和用途治理内容。
7. 可以保留少量作者判断，但必须服务于概念澄清；不要把点评写成情绪输出。

详细维护规则见[贡献与维护约定](docs/23-maintenance-guide.md)。
